    ...
    <div className="p-2 group">
      <div className="relative overflow-hidden mb-3">
        {product.soldOut && (
          <div className="absolute top-0 left-0 z-10 bg-red-600 px-2 py-1 rounded-lg">
             <span className="text-white text-xs font-semibold tracking-wider">SOLD OUT</span>
          </div>
        )}
        <div className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
           <div className="bg-gray-500 p-2 rounded-full shadow-md cursor-pointer hover:bg-[#bd9951] hover:text-white transition-colors">
               <Image src="/assets/icons/heart.svg" alt="wishlist" width={16} height={16} className="w-4 h-4" />
           </div>
        </div>
      ...
      <div className="text-left">
        <h6 className="mb-2 text-sm font-medium line-clamp-2 h-[40px] leading-tight">
          <Link href={`/product/${product.id}`} className="hover:text-[#bd9951] transition-colors">{product.name}</Link>
        </h6>
        ...
        <div className="flex items-center gap-2 text-sm">
          <span className="font-bold text-[#bd9951]">₹ {product.price.toFixed(2)}</span>
          <span className="line-through text-gray-400 text-xs">₹ {product.mrp.toFixed(2)}</span>
