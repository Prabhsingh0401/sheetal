...
const NewArrivals = () => {
  ...
  const getDisplayPrice = (product: Product) => { // Add type to product argument
    if (!product.variants || product.variants.length === 0 || !product.variants[0].sizes || product.variants[0].sizes.length === 0) {
    ...
    const discount = mrp > price ? `${Math.round(((mrp - price) / mrp) * 100)}% OFF` : '';
    return {
              ...
              <div className="flex gap-4">
                ...
                {loading ? (
                    Array.from({ length: 3 }).map((_, index) => (
                        <div key={index} className="flex-shrink-0 w-[85%] sm:w-[48%] lg:w-[33.333%] animate-pulse">
                    ...
                    products.map((product) => {
                        ...
                        const displaySize = firstVariant?.sizes?.[0]?.name || 'N/A';
                            ...
                            >
                                <div className="rounded-xl overflow-hidden group ml-2">
                                    <div className="relative aspect-[3/4] overflow-hidden">
                                        ...
                                        />
                                        <Image
                                            src={getApiImageUrl(product.hoverImage?.url || product.mainImage?.url)}
                                            alt={product.name}
                                            width={400}
                                            height={533}
                                    ...
                                    <div className="p-4 text-center">
                                        ...
                                        <div className="text-xs text-gray-600">
                                            <span className="font-bold text-black">Size:</span> {displaySize}
                                        </div>
                                        ...
                                        {displayPrice.discount && <span className="text-lg text-[#281b00] font-bold">
                                            {displayPrice.price}
                                        </span>}
                                        <span className={`text-xs text-gray-400 ${displayPrice.discount ? "line-through" : "text-lg text-[#281b00] font-bold"}`}>
                                            {displayPrice.mrp}
                                        </span>
                                        </div>
                                        {displayPrice.discount && <span className="text-xs text-[#B78D65] font-bold">
